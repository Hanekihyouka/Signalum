...
// instance/kubejs/server_scripts/utils/remove_recipes_from_banlist.js
...
function sendMessageAndCancel(/** @type {$ItemClickedKubeEvent_} */event, type) {
  switch (type) {
    case "item":
      event.player.setStatusMessage(Text.red("此物品已被禁用!"))
      event.cancel()
      break
    default:
      break
...
let addSignToPos = (/** @type {$ServerLevel_} */level, /** @type {$BlockPos_} */pos) => {
  ...
  let nbt = {is_waxed: 1, front_text: {has_glowing_text: 1, color: "black", messages: [namespace, path, '"已被禁用。"', "server"]}}
